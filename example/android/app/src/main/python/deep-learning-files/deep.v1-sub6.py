...
data =  np.array(data)
print('[Sam] Video loaded...', len(data))
...
print('[Sam] Train test split performed...')
...
print('[Sam] Train data loaded in TF...')
...
#         # pdb.set_trace()
#         print('batches', batch_x.shape, batch_y.shape)
...
import os
sub_name = 'sub6'
if not os.path.exists(f'./nn_weights/{sub_name}'):
    os.mkdir(f'./nn_weights/{sub_name}')
def train_nn(neural_net1, neural_net2, train_data):
    min_val_loss = float('inf')  # Initialize the minimum validation loss to a large value
    ...
    for step, (batch_x, batch_y) in enumerate(train_data.take(training_steps), 1): 
        ...
        if step % 10 == 0:
            ...
            if current_val_loss < min_val_loss:
                ...
                if not os.path.exists(f'./nn_weights/{sub_name}/mid-saves/'):
                    os.mkdir(f'./nn_weights/{sub_name}/mid-saves/')
                
                print(f'Saving model with validation loss: {min_val_loss}\n')
                neural_net1.save_weights(f'./nn_weights/{sub_name}/mid-saves/weights')
...
neural_net1.save_weights(f'./nn_weights/{sub_name}/weights')
print(f'Model weights saved in {datetime.today() - t1}\n\n\n\n')
...
neural_net1.load_weights(f'./nn_weights/{sub}/weights')
